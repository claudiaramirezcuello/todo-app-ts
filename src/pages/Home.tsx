...
const Home = (): JSX.Element => {
  ...
  const [filterSelected, setFilterSelected] = useState<FilterValue>(
    TODO_FILTERS.ALL,
  ); // el filtro por defecto es 'all', setFilterSelected actualiza el filtro seleccionado
  const [userName] = useState("Clàudia");

  useEffect(() => {
    const storedTodos = localStorage.getItem("myTodos");
    if (storedTodos && JSON.parse(storedTodos) !== todos)
  ...
  const setTodosRef = useRef(false);
  useEffect(() => {
    if (setTodosRef.current) setTodosLocalStorage();
    setTodosRef.current = true;
  }, [todos]);
  ...
  const handleRemoveAllCompleted = (): void => {
    ...
    const confirmed = window.confirm(
      "¿Estás seguro de querer borrar todas las tareas completadas?",
    );
  ...
  return (
    <div className="todoapp">
      <Header onAddTodo={handleAddTodo} userName={userName} />
      <Button
        onClick={handleClick}
        text="Click me"
        className="myButton"
